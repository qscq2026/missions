...
| 🔧 **工人 Worker** | 用 TDD 实现功能，使用文件范围的上下文——不跨角色推理泄露 |
...
5. **经验预加载** — 每个角色在执行任务前自动加载相关历史经验，按检测到的角色和任务类型过滤，避免重复踩坑、复用已验证的模式
6. **自我改进循环** — 每次任务完成后，智能体审查已记录的经验以持续改进：规避过往阻塞、复用成功模式、优化 token 使用
...
编排者会问最多 3 个问题，按优先级排序：先确认里程碑划分，再问影响断言设计的技术选择，最后是工具细节。回答后**退后一步**——智能体会处理其余所有工作。
...
| 功能 | 说明 |
|------|------|
| 🎯 **Coverage Map 里程碑检测** | PR Author 通过 `coverage_map` key→feature ID 精确匹配触发，杜绝误报 |
| 🧩 **优先级排序的澄清提问** | Orchestrator 按严格优先级提问：Q1 里程碑范围 → Q2 断言影响 → Q3 工具细节 |
| 🧹 **文件范围上下文** | Worker 使用文件范围上下文（File-scoped context），只读角色协议和相关文件 |
| 📋 **合约模板** | 新增 `assets/contract-template.md`，为 Orchestrator 提供 CONTRACT.md 起始结构 |
| 🔗 **层次化修复追踪** | Fix card 追踪 `root_feature` + `fix_number`，跨多轮修复保证审计链完整性 |
| ⚡ **优化重启序列** | 状态检测移至 config/experience 加载之前，经验按检测到的角色过滤 |
| 🧠 **智能经验策展** | Critical 严重度首次即记录；major/minor 需出现≥2次才记录 |
| 🚀 **自动 Seed 引导** | `bootstrap.sh` 自动拷贝 seed 经验和合约模板到项目 |

### 此前在 v1.1 中

| 功能 | 说明 |
|------|------|
| 🔄 **REACTIVATION 协议** | 智能体崩溃恢复与会话重启 |
| 🛡️ **安全审计角色** | 用于安全敏感功能审查的内置角色 |
| 📝 **上下文编写角色** | 用于文档和知识产物的可扩展角色 |
...
│       └── experience/            ← 示例经验卡片
├── assets/
│   ├── contract-template.md        ← CONTRACT.md 起始结构模板
...
| 未生成 PR | 里程碑未按 `coverage_map` 完成 | 验证该里程碑的所有 feature ID 是否已存在于 `05-done/` |
...
智能体会自动：
1. 检测已存在的 `.missions/` 状态 **(第一步 — 在读取 config 或经验之前)**
2. 读取 `.missions/config.yaml` 获取项目配置
3. 读取 `.missions/logs/experience/INDEX.md` 获取历史上下文
4. 按**检测到的角色**（Worker/Validator/Fix）和任务类型过滤经验——只加载最相关的 3 条
5. 读取 `.missions/03-running/*.md`（如有）恢复部分任务上下文
6. 从中断处继续执行，经验已应用
...
| 维度 | 原版 Missions | Missions Skill (v2.0) |
|------|--------------|----------------------|
| 规范 | 无 | ✅ **agentskills.io** 兼容 |
| 配置 | 无 | ✅ **config.yaml** 驱动 |
| 模板 | 硬编码 | ✅ **可覆盖** assets/（11 个模板，含 contract-template） |
| 角色 | 3 个固定 | ✅ **4 + 2 可扩展**（安全审计、上下文编写） |
| 钩子 | 无 | ✅ **scripts/** 生命周期 |
| 里程碑检测 | 手动 | ✅ **coverage_map** 自动检测 |
| 恢复机制 | 无 | ✅ **REACTIVATION.md** 崩溃恢复协议 |
| 经验系统 | 无 | ✅ **经验卡片**（模板 + 索引 + 示例） |
| 经验策展 | 全记录 | ✅ **严重度阈值**（critical→立即，major/minor→≥2次） |
| 修复追踪 | 单层 parent ID | ✅ **层次化**（root_feature + fix_number + 直接父级） |
| Worker 上下文 | "Fresh start" | ✅ **文件范围上下文**（只读角色协议+相关文件） |
| 澄清顺序 | 任意 | ✅ **优先级排序**（Q1 milestone → Q2 断言 → Q3 工具） |
| 重启序列 | config→经验→检测 | ✅ **检测→config→按角色过滤经验** |
